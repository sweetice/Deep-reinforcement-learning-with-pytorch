...
class Agent():
    ...
    def __init__(self):
        ...
        self.seeds = int(np.random.choice(np.linspace(0, 3, 4)))
        self.state = self.birth_state_space[self.seeds]
    ...
    def is_in_cross_road(self, state):
        x, y = state[0], state[1]
        if x in [GRIDWORLDSIZE[0]//2, GRIDWORLDSIZE[0]//2-1] or y in [GRIDWORLDSIZE[1]//2, GRIDWORLDSIZE[1]//2-1]:
            return True
        else:
            return False

    def get_action_space(self,state):
        action_space = ["up", "down","left", "right", "stop"]
        if state in self.cross_road_space:
            if state == (6,6):
                action_space.remove('up')
                action_space.remove('right')
            if state == (6,7):
                action_space.remove('up')
                action_space.remove('left')
            if state == (7,6):
                action_space.remove('down')
                action_space.remove('right')
            if state == (7,7):
                action_space.remove('down')
                action_space.remove('left')
    ...
    def get_action(self):
        action_space = self.get_action_space(self.state)
